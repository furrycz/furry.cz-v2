...
class IntercomPresenter extends BasePresenter
{
	...
	public function renderDefault($name)
	{
		...
		{
			throw new BadRequestException("Nejste schváleným uživatelem");
		}
		$database = $this->context->database;
		
		$allUsers = array();
		$allUserId = NULL;
		$allUserWithInfo = NULL;
		$allUserName = NULL;
		...
		if($name != NULL)
		{
			...
			$userData = $allUserWithInfo[$allUserName[$name]];
			$this->template->idAdresser = $ID;
			$this->template->selUser = $userData[0];
			$this->template->selUserNick = $name;
			$this->template->selUserLastLogin = $userData[2];
			$this->template->selAvatar = $userData[1];
		...
		$allMessages = NULL;
		$msgFrom = NULL;
		$messageActualShow = NULL;
		$messageCount = NULL;
		$notReaded = NULL;

		$messages = $database
			->table('PrivateMessages')
			->where("(SenderId = ? OR AddresseeId = ?) AND Deleted=0", $this->user->identity->id, $this->user->identity->id)
			->order('TimeSent DESC');
		...
		foreach($messages as $message)
		{
			...
			if($allUserName[$name_] == $message["SenderId"] or $allUserName[$name_] == $message["AddresseeId"])
			{
				...
				if($name_==$allUserId[$message["SenderId"]])
				{
					$typ=1;
				}
				else
				{
					$typ=0;
				} // 1 -> odesilatel on; 0 -> odesilatel ja
		...
		{
			// FIXME: This is awful
			$allMessages[] = array(0,"<div style='padding:5px;'></div>Žádná zpráva nebyla nalezena!","","","","","");
		}
		...
		$this->template->setParameters(array(
			...
			"messageActualShow" => $messageActualShow,
	...
	/** Deletes PrivateMessage by Id (single) or Username (bulk)
	* @param string $name Username. Only needed if $id isn't used.
	* @param int    $id   Message Id. Use -1 as NULL value (username will be used)
	*/
	public function handleDelete($name, $id)
	{
		...
		if($id==-1)
		{
			...
			$database
				->table('PrivateMessages')
				->where("(SenderId = ? AND AddresseeId = ?) OR (SenderId = ? AND AddresseeId = ?)",$this->user->identity->id,$allUserName[$name],$this->user->identity->id,$allUserName[$name])
	...
	public function handleExport($name)
	{
		...
		$messages = $database
			->table('PrivateMessages')
			->where("(SenderId = ? OR AddresseeId = ?) AND (SenderId = ? OR AddresseeId = ?) AND Deleted=0",$this->user->identity->id, $this->user->identity->id,$user["Id"],$user["Id"])
			->order('TimeSent DESC');
		...
		foreach($messages as $message)
		{
			...
			if($message["Read"]==1)
			{
				$read="<span style='color:green'>[Přečteno]</span>";
			}
			else
			{
				$read="";
